...
//
// The ClipVault popup: clipboard history + notes/accounts.
//
...
/**
 * First `maxLines` lines of `text`, without leading/trailing blank lines.
 * Each line is capped so huge single-line blobs stay cheap to lay out; the
 * visual (wrapped) line limit is applied later by clampLines().
...
function previewText(text, maxLines) {
    ...
    while (lines.length && !lines[lines.length - 1].trim())
        lines.pop();
    const kept = lines.slice(0, maxLines).map(l => l.slice(0, PREVIEW_LINE_CHARS));
    const cut = lines.length > maxLines || lines.some(l => l.length > PREVIEW_LINE_CHARS);
    return {text: kept.join('\n'), cut};
}

/**
 * Limits a wrapping label to `maxLines` visual lines, ending in "…" when
 * the text doesn't fit. ClutterText can't do this itself, so the text is
 * measured with Pango once the label knows its width.
 */
function clampLines(label, preview, maxLines, defer) {
    const ct = label.clutter_text;
    let lastWidth = -1;
    const update = () => {
        const width = ct.get_width();
        if (width <= 0 || width === lastWidth)
            return;
        lastWidth = width;
        // Measure with the label's own layout, already wrapped to its width.
        if (ct.text !== preview.text)
            ct.text = preview.text;
        const layout = ct.get_layout();
        if (layout.get_line_count() <= maxLines) {
            ct.text = preview.cut ? `${preview.text}…` : preview.text;
            return;
        }
        // Byte offset (UTF-8) where the first hidden line starts. A layout
        // iterator is used because GJS can't read PangoLayoutLine bitfields.
        const iter = layout.get_iter();
        for (let i = 0; i < maxLines; i++)
            iter.next_line();
        const bytes = new TextEncoder().encode(preview.text);
        const kept = new TextDecoder().decode(bytes.slice(0, iter.get_index()));
        ct.text = `${kept.slice(0, -2).trimEnd()}…`;
    };
    // Changing the text during allocation would re-queue a relayout.
    ct.connect('notify::allocation', () => defer(update));
}
...
function prettyAccelerator(accel) {
    const names = {super: 'Super', alt: 'Alt', shift: 'Shift', control: 'Ctrl', primary: 'Ctrl', ctrl: 'Ctrl'};
    const mods = [...accel.matchAll(/<(\w+)>/g)].map(m => names[m[1].toLowerCase()] ?? m[1]);
    const key = accel.replace(/<\w+>/g, '');
    return [...mods, key.length === 1 ? key.toUpperCase() : key].join('+');
}

/** Appends the configured shortcut, e.g. "Generate a password (Super+Alt+P)". */
function withShortcut(text, settings, key) {
    const accel = settings.get_strv(key)[0];
    return accel ? `${text} (${prettyAccelerator(accel)})` : text;
}
...
export class ClipboardPopup {
     ...
     * @param {string} [options.newAccountPassword] open the account editor with this password
     */
    open(tab = TAB_HISTORY, position = null, options = {}) {
        if (this.isOpen)
            return;
        ...
        this._switchTab(tab);
        if (options.newAccountPassword)
            this._openEditor(null, {kind: 'account', password: options.newAccountPassword});
        this._box.opacity = 0;
    ...
    _destroyActors() {
        ...
        this._tooltip = null;
        // Drop key focus first so a focused entry isn't destroyed mid-input.
        if (this._backdrop?.contains(global.stage.get_key_focus()))
            global.stage.set_key_focus(null);
        // While the editor is shown the list is detached from the tree, so
        // destroying the backdrop wouldn't reach it.
        if (this._scroll && !this._scroll.get_parent())
    ...
    /**
     * St has no tooltips, so show a small label after hovering for a moment.
     * `text` may be a function to reflect the current state (e.g. pin/unpin).
     */
    _tip(actor, text) {
        const getText = typeof text === 'function' ? text : () => text;
        actor.accessible_name = getText();
        actor.connect('notify::hover', () => {
            if (actor.hover)
                this._scheduleTooltip(actor, getText);
            else
                this._hideTooltip();
        });
    ...
    _scheduleTooltip(actor, getText) {
        this._hideTooltip();
        this._tooltipOwner = actor;
        this._tooltipTimeoutId = this._timeout(TOOLTIP_DELAY_MS, () => {
            this._tooltipTimeoutId = 0;
            this._showTooltip(actor, getText());
        });
    }

    _showTooltip(actor, text) {
        if (!this.isOpen || !actor.get_stage())
            return;
        if (!this._tooltip) {
            this._tooltip = new St.Label({style_class: 'cv-tooltip'});
            this._backdrop.add_child(this._tooltip);
        }
        ...
        const [, tw] = tip.get_preferred_width(-1);
        const [, th] = tip.get_preferred_height(tw);
        let x = ax + (aw - tw) / 2;
        let y = ay + ah + 6;
        if (y + th > global.stage.height)
            y = ay - th - 6;
        x = Math.max(4, Math.min(x, global.stage.width - tw - 4));
        tip.set_position(Math.round(x), Math.round(y));
        this._backdrop.set_child_above_sibling(tip, null);
        tip.show();
        tip.opacity = 0;
        tip.ease({opacity: 255, duration: 100, mode: Clutter.AnimationMode.EASE_OUT_QUAD});
    }

    _hideTooltip() {
        if (this._tooltipTimeoutId) {
            GLib.source_remove(this._tooltipTimeoutId);
            this._timeouts.delete(this._tooltipTimeoutId);
            this._tooltipTimeoutId = 0;
        }
    ...
    _build() {
        ...
        this._addButton = this._tip(iconButton('list-add-symbolic'), _('New note or account (Ctrl+N)'));
        this._addButton.connect('clicked', () => this._openEditor(null));
        this._clearButton = this._tip(iconButton('edit-clear-all-symbolic'),
            _('Clear history (pinned items are kept)'));
        this._clearButton.connect('clicked', () => this._clearHistory());
        this._privateButton = this._tip(iconButton('view-conceal-symbolic', 'cv-toggle'),
            () => (settings.get_boolean('private-mode')
                ? _('Private mode is on: click to resume recording')
                : _('Private mode: stop recording what you copy')));
        this._privateButton.connect('clicked', () => {
            settings.set_boolean('private-mode', !settings.get_boolean('private-mode'));
            this._syncPrivate();
            this._refresh();
        });
        const generateButton = this._tip(iconButton('dialog-password-symbolic'),
            () => withShortcut(_('Generate a password and paste it'), settings, 'generate-password'));
        generateButton.connect('clicked', () => this._ctrl.generatePassword());
        const prefsButton = this._tip(iconButton('emblem-system-symbolic'), _('Preferences'));
        prefsButton.connect('clicked', () => {
            this.close();
            this._ctrl.openPreferences();
        });
        [this._addButton, this._clearButton, generateButton, this._privateButton, prefsButton]
    ...
    /**
     * Positions the popup. It prefers to open right below the
    ...
    _makeTab(iconName, label, tab) {
        ...
        return this._tip(button, () => (tab === TAB_HISTORY
            ? withShortcut(_('Everything you copied'), this._ctrl.settings, 'toggle-history')
            : withShortcut(_('Saved notes and accounts'), this._ctrl.settings, 'toggle-notes')));
    }
    ...
    _refresh() {
        ...
        for (const item of items) {
            // Section headers make pinned items stand out at a glance.
            const itemSection = item.pinned ? 'pinned' : 'rest';
            if (hasPinned && itemSection !== section) {
                section = itemSection;
                this._list.add_child(this._sectionHeader(section));
            }
            const row = this._tab === TAB_HISTORY ? this._historyRow(item) : this._noteRow(item);
            if (item.pinned)
                row.add_style_class_name('cv-row-pinned');
    ...
    _sectionHeader(section) {
        const box = new St.BoxLayout({style_class: `cv-section cv-section-${section}`, x_expand: true});
        if (section === 'pinned') {
            box.add_child(new St.Icon({icon_name: 'view-pin-symbolic', icon_size: 12, y_align: Clutter.ActorAlign.CENTER}));
            box.add_child(new St.Label({text: _('Pinned'), y_align: Clutter.ActorAlign.CENTER}));
        } else {
            box.add_child(new St.Label({
                text: this._tab === TAB_HISTORY ? _('Recent') : _('Other'),
                y_align: Clutter.ActorAlign.CENTER,
            }));
        }
        return box;
    ...
    _historyRow(item) {
        ...
        if (item.type === 'text') {
            const maxLines = this._ctrl.settings.get_int('preview-lines');
            const preview = previewText(item.text, maxLines);
            if (!preview.text.trim())
                preview.text = _('(whitespace)');
            ...
            // St.Label ellipsizes by default, which collapses the text to a
            // single line whenever it's briefly given too little height.
            // clampLines() does the truncation instead.
            label.clutter_text.ellipsize = Pango.EllipsizeMode.NONE;
            clampLines(label, preview, maxLines, fn => this._timeout(0, fn));
        ...
        side.add_child(this._tip(time, _('Copied on %s').format(new Date(item.time).toLocaleString())));

        if (item.type === 'text') {
            const save = this._tip(iconButton('document-save-symbolic'), _('Save as a note (Ctrl+S)'));
            save.connect('clicked', () => this._saveAsNote(item));
            side.add_child(save);
        }
        const pin = this._tip(iconButton('view-pin-symbolic', 'cv-toggle'),
            item.pinned ? _('Unpin (Ctrl+P)') : _('Pin: keep it at the top, even after clearing (Ctrl+P)'));
        if (item.pinned)
            pin.add_style_pseudo_class('checked');
        pin.connect('clicked', () => this._ctrl.store.togglePin(item.id));
        side.add_child(pin);

        const del = this._tip(iconButton('window-close-symbolic'), _('Remove from history (Del)'));
    ...
    _noteRow(note) {
        ...
        if (isAccount) {
            ...
            if (note.username) {
                const user = this._tip(textButton(_('Username')), _('Paste the username (Ctrl+U)'));
                user.connect('clicked', () => this._activateUsername(note, {paste: true}));
                actions.add_child(user);
            }
            const pass = this._tip(textButton(_('Password')), _('Paste the password (Enter)'));
        ...
        const side = new St.BoxLayout({style_class: 'cv-row-side', y_align: Clutter.ActorAlign.CENTER});
        const pin = this._tip(iconButton('view-pin-symbolic', 'cv-toggle'),
            note.pinned ? _('Unpin (Ctrl+P)') : _('Pin: keep it at the top (Ctrl+P)'));
        if (note.pinned)
            pin.add_style_pseudo_class('checked');
        pin.connect('clicked', () => this._ctrl.store.toggleNotePin(note.id));
        side.add_child(pin);

        const edit = this._tip(iconButton('document-edit-symbolic'), _('Edit (Ctrl+E)'));
        edit.connect('clicked', () => this._openEditor(note));
        side.add_child(edit);

        const del = this._tip(iconButton('user-trash-symbolic'), _('Delete (press twice to confirm)'));
    ...
    _openEditor(note, preset = {}) {
        ...
        const entry = (hint, text = '') => {
            const e = new St.Entry({
                style_class: 'cv-entry', hint_text: hint, text, can_focus: true, x_expand: true,
            });
            // An editable ClutterText reports its cursor to the input method
            // even when unfocused (e.g. when the text is filled in), which
            // makes Clutter log criticals. Only be editable while focused.
            const ct = e.clutter_text;
            ct.editable = false;
            ct.connect('key-focus-in', () => {
                ct.editable = true;
            });
            ct.connect('key-focus-out', () => {
                ct.editable = false;
            });
            return e;
        };
        ...
        this._fPass = entry(this._passwordLoaded ? _('password') : _('Loading from keyring…'), preset.password ?? '');
        ...
        let hidden = true;
        const reveal = this._tip(iconButton('view-reveal-symbolic', 'cv-toggle'),
            () => (hidden ? _('Show password') : _('Hide password')));
        ...
        const generate = this._tip(iconButton('view-refresh-symbolic'), _('Generate a strong password'));
        generate.connect('clicked', () => {
            this._passwordLoaded = true;
            this._fPass.set_text(Passwords.generatePassword(Passwords.optionsFromSettings(this._ctrl.settings)));
        });
        passRow.add_child(generate);
        this._fPassBox = field(Secrets.isAvailable()
    ...
    async _saveEditor() {
        ...
        try {
            ...
            // The extension may have been disabled (e.g. screen locked) meanwhile.
            if (!this._ctrl.store)
                return;
